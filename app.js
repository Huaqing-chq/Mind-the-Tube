 ...
 * @param {HTMLElement} statusContainer - 线路卡片的容器元素
 * @param {HTMLElement} loadingMessage - 初始加载消息元素
 */
async function fetchTubeStatus(statusContainer, loadingMessage) {
    if (!statusContainer) {
    ...
    try {
        ...
        lines.forEach(line => {
            renderLine(line, statusContainer);
        });
        ...
        renderError("无法获取实时状态。请检查您的网络连接并重试。", statusContainer, loadingMessage);
    }
}

/**
 * @description 渲染单条线路卡片 (概览)
 * @param {object} line - 单条线路数据
 * @param {HTMLElement} statusContainer - 线路卡片的容器元素
 */
function renderLine(line, statusContainer) {
    ...
    // 关键：将 toggleLineDetails 提升到 window 作用域，以便在 card.innerHTML 中调用
...
function renderStationList(stations, detailsDiv) {
    ...
    // 过滤掉非地铁站点的条目
    const validStati    'northern': '北线 (Northern)',
    'piccadilly': '皮卡迪利线 (Piccadilly)',
    'victoria': '维多利亚线 (Victoria)',
    'waterloo-city': '滑铁卢及城市线 (W&C)',
    'elizabeth-line': '伊丽莎白线 (Elizabeth)'
...
async function fetchDisruptedStations() {
    try {
        ...
        window.disruptedStationIds = Array.from(disruptionSet);
    } catch (error) {
        console.error("无法获取站点中断信息:", error);
        window.disruptedStationIds = []; 
    }
}
...
async function fetchTubeStatus() {
    ...
    try {
        ...
        // 关键：清除容器，这会移除 "Loading..."
        window.statusContainer.innerHTML = '';
        ...
        // 首次加载时，在后台获取中断信息
        if (window.disruptedStationIds === null) {
            fetchDisruptedStations();
        }
...
function renderLine(line) {
    if (!window.statusContainer) return;
    ...
    window.statusContainer.appendChild(card);
}
